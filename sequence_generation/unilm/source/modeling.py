...
from sequence_generation.unilm.source.convert_state_dict import get_checkpoint_from_transformer_cache, state_dict_convert
...
def join_path(model_name):
    return BERT_DOWNLOAD_URL_PREFIX + "/" + model_name + "/" + MODEL_FILE


xBERT_PRETRAINED_MODEL_ARCHIVE_MAP = {
...
for l in [2, 4, 6, 8, 10, 12]:
    for h in [128, 256, 512, 768]:
        ...
        xBERT_PRETRAINED_MODEL_ARCHIVE_MAP[model_name_suffix] = join_path(model_name)
...
class BertPreTrainedForSeq2SeqModel(BertPreTrainedModel):
    ...
    supported_convert_pretrained_model_archive_map = {
        ...
        "xbert": xBERT_PRETRAINED_MODEL_ARCHIVE_MAP,
        'minilm2-bert-base': {},
        'minilm2-bert-large': {},
        'minilm2-roberta': {},
    }
        ...
        if model_type is not None and "state_dict" not in kwargs:
            if model_type in cls.supported_convert_pretrained_model_archive_map:
                ...
                if pretrained_model_name_or_path in pretrained_model_archive_map:
                    ...
                    if model_type in ['minilm', 'unilm', 'xbert'] and not any(['roberta' in x for x in state_dict.keys()]):
