...
def convert_example(example):
    if "src" not in example:
        text = [example['title'], example['abstract']]
        text = ' {} '.format(tokenizer.sep_token).join(text)
...
def load_and_cache_examples(
        example_file, tokenizer, local_rank, cached_features_file,
        shuffle=True, threads=1, tqdm_enabled=True
):
